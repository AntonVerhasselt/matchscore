...
function getPrimaryColor(isHome: boolean): string {
  if (isHome) {
    return "#2563eb";
  }
  return "#dc2626";
...
export function createPlaceholderCrestSvg(bindingKey: ImageBindingKey): string {
  const isHome = bindingKey === "homeClubLogo";
  const primary = getPrimaryColor(isHome);
...
export function createPlaceholderCrestDataUrl(bindingKey: ImageBindingKey): string {
  return `data:image/svg+xml;utf8,${encodeURIComponent(createPlaceholderCrestSvg(bindingKey))}`;
}

...
import { ORG_INVITATION_EXPIRES_IN_DAYS } from "../../emails/OrganizationInvitationEmail";
...
export const sendOrganizationInvitationEmail = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      const { html, subject } = await renderEmail("org-invitation", {
      ...
      console.error("Failed to send organisation invitation email", {
        to: args.to,
        locale,
        error,
      });
